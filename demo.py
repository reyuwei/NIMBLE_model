...
if __name__ == "__main__":
    ...
    pm_dict_name = r"assets/model/new/pm_dict_9173.pkl"
    tex_dict_name = r"assets/model/new/tex_dict.pkl"
    ...
    if os.path.exists(r"assets/model/new/NIMBLE_MANO_VREG.pkl"):
        nimble_mano_vreg = np.load("assets/model/new/NIMBLE_MANO_VREG.pkl", allow_pickle=True)
        nimble_mano_vreg = batch_to_tensor_device(nimble_mano_vreg, device)
    else:
        nimble_mano_vreg=None

    layer = NIMBLELayer(pm_dict, tex_dict, device, use_pose_pca=True, pose_ncomp=30, shape_ncomp=20, nimble_mano_vreg=nimble_mano_vreg)
    bn = 2
    pose_param = torch.rand(bn, 30) * 2 - 1 
    shape_param = torch.rand(bn, 20) * 2 - 1 
    tex_param = torch.rand(bn, 10) - 0.5

    skin_v, muscle_v, bone_v, bone_joints, tex_img = layer.forward(pose_param, shape_param, tex_param, handle_collision=True)

    skin_p3dmesh = Meshes(skin_v, layer.skin_f.repeat(bn, 1, 1))
    muscle_p3dmesh = Meshes(muscle_v, layer.muscle_f.repeat(bn, 1, 1))
    bone_p3dmesh = Meshes(bone_v, layer.bone_f.repeat(bn, 1, 1))
    
    skin_p3dmesh = smooth_mesh(skin_p3dmesh)
    muscle_p3dmesh = smooth_mesh(muscle_p3dmesh)
    bone_p3dmesh = smooth_mesh(bone_p3dmesh)

    skin_mano_v = layer.nimble_to_mano(skin_v, is_surface=True)
